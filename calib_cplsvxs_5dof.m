function [error, IAC, K] = calib_cplsvxs_5dof(cps, lss, vxs, K0)
...
for i = 1:size(cps, 2)
    cp = cps(:,i);
    coef = [coef;     cp(1)*cp(1) cp(2)*cp(2)   2*cp(1)*cp(2)  2*cp(1)  2*cp(2)        1];     % iT w i = 0
end
...
for i = 1:size(lss,2)
    ...
    vx = vxs(:,i);
    coef = [ coef;
             0          -ls(3)*vx(2)       -ls(3)*vx(1)              ls(2)*vx(1)       ls(2)*vx(2)-ls(3)      ls(2); % ls*(w vx) = 0
        ls(3)*vx(1)          0              ls(3)*vx(2)          ls(3)-ls(1)*vx(1)       -ls(1)*vx(2)        -ls(1);
       -ls(2)*vx(1)      ls(1)*vx(2)   -ls(2)*vx(2)+ls(1)*vx(1)        -ls(2)                ls(1)              0
];
end
% normalize each row
...
C = [v(1,end)  v(3,end)   v(4,end)
     v(3,end)  v(2,end)   v(5,end)
...
es = abs(K(1,2)-K0(1,2))/K0(1,2);

error= 100*[eaf ef es eu ev]
