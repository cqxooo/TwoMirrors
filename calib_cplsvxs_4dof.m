function [error, IAC, K] = calib_cplsvxs_4dof(cps, lss, vxs, K0)
...
for i = 1:size(lss,2)
    ...
    vx = vxs(:,i);
    coef = [ coef;
             0          -ls(3)*vx(2)        ls(2)*vx(1)       ls(2)*vx(2)-ls(3)      ls(2); % ls*(w vx) = 0
        ls(3)*vx(1)          0          ls(3)-ls(1)*vx(1)       -ls(1)*vx(2)        -ls(1);
       -ls(2)*vx(1)      ls(1)*vx(2)          -ls(2)                ls(1)              0
];
end
% normalize each row
